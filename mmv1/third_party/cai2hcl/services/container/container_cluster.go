...
func (c *ContainerClusterConverter) Convert(assets []*caiasset.Asset) ([]*common.HCLResourceBlock, error) {
	...
	for _, asset := range assets {
		...
		if asset.Resource != nil && asset.Resource.Data != nil {
			newBlocks, err := c.convertResourceData(asset)
			if err != nil {
				return nil, err
			}
			if newBlocks != nil {
...
func (c *ContainerClusterConverter) convertResourceData(asset *caiasset.Asset) ([]*common.HCLResourceBlock, error) {
	...
	if location == "" {
		...
		location = common.ParseFieldValue(asset.Name, "zones")
	}
	if location == "" {
		// If zones not found, try regions (for regional clusters)
		location = common.ParseFieldValue(asset.Name, "regions")
	}
	...
	hasOnlyDefaultPool := len(nodePools) == 1 && nodePools[0] != nil && nodePools[0].Name == "default-pool"
	...
	if len(nodePools) > 0 && !autopilot && !hasOnlyDefaultPool {
...
func (c *ContainerClusterConverter) convertClusterData(cluster *container.Cluster, project, location, clusterName string, nodePools []*container.NodePool) (*common.HCLResourceBlock, error) {
	if cluster == nil {
	...
	if hasSeparateNodePools {
		...
		} else if flattened := flattenNodeConfig(nodePools[0].Config); flattened != nil {
			hclData["node_config"] = flattened
		}
	...
	if len(cluster.Locations) > 0 {
		...
		locations := zonesAndLocations.List()
		zonalCluster := len(locations) == 1 && locations[0] == cluster.Location
		if len(zonesAndLocations.List()) > 0 && !zonalCluster {
	...
	ctyVal, err := common.MapToCtyValWithSchema(hclData, c.clusterSchema)
	if err != nil {
...
func (c *ContainerClusterConverter) convertNodePoolData(nodePool *container.NodePool, cluster *container.Cluster, project, location string) (*common.HCLResourceBlock, error) {
	if nodePool == nil {
	...
	ctyVal, err := common.MapToCtyValWithSchema(hclData, c.nodePoolSchema)
	if err != nil {
...
func flattenLoggingConfig(config *container.LoggingConfig) []interface{} {
	if config == nil || config.ComponentConfig == nil || len(config.ComponentConfig.EnableComponents) == 0 {
		// default components are "SYSTEM_COMPONENTS" and "WORKLOADS"
		return nil
	}
